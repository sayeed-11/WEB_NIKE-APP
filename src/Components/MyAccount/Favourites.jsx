...
const FavouriteData = () => {
    ...
    return (
        <div className='px-2 sm:px-5'>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 py-10'>
                {
                    wishlistData.map((data, index) => {
                        return (
                            <div key={index} className='overflow-hidden border-[1px] border-black/[0.1] font-nike rounded-md shadow-sm bg-white relative'>
                                <div className={`absolute z-40 ${isEdit ? "block" : "hidden"}`}>
                                    <i onClick={() => {
                                    ...
                                    }} className={`fa-${inArray(data.id, wishlistArray) ? "solid" : "regular"} fa-heart w-9 sm:w-12 aspect-square grid place-items-center rounded-ee-lg  text-sm scale-[1]  text-white bg-black shadow-md`} />
                                </div>
                                <img className='-translate-y-16' src={data.colorsAvailable[0].images[0]} alt="" />
                                <div className='px-2 py-2 -translate-y-16 -mb-16 sm:space-y-3'>
                                    <div>
                                        <h1>{data.colorsAvailable[0].name}</h1>
                                        <p className='text-xs line-clamp-1'>{data.type}</p>
                                    </div>
                                    <div className='flex flex-col-reverse sm:flex-row justify-between gap-y-1'>
                                        <button className='bg-black text-white px-3 py-2 text-xs rounded-sm'>Select size</button>
                                        <p className='text-sm sm:text-base'>MRP : ₹ {data.colorsAvailable[0].price}</p>
                                    </div>
                                </div>
